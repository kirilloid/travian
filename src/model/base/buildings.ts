...
export class BMethods<C extends BConfigured = BConfigured> {
    ...
    public time(this: BMethods & C, lvl: number) {
        return this.t(lvl);
    }
    public cost(this: BMethods & C, lvl: number): res {
        return this.c.map((res: number) => round5(res * this.k ** (lvl - 1))) as res;
    }
    public upkeep(this: BMethods & C, lvl: number) {
        return lvl === 1 ? this.u : Math.round((5 * this.u + lvl - 1) / 10);
    }
    public culture(this: BMethods & C, lvl: number) {
        return Math.round(this.cp * 1.2 ** lvl);
    }
    public benefit(this: BMethods & C, lvl: number) {
        return this.f(lvl);
    }
    public nameKey(this: BMethods & C) {
        return 'objects.buildings.names.' + (this.nt || 'b_' + (this.id + 1));
    }
    public descriptionKey(this: BMethods & C) {
