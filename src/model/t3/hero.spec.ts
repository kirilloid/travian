...
tape('hero (T3)', t => {
    t.test('leggionnaire', t => {
        ...
        const hero = new Hero(unit);
        t.test('level 0', t => {
            t.deepEqual(hero.getCombat(), { a: 50, di: 60, dc: 85 }, 'combat');
        ...
        t.test('level 40', t => {
            ...
            t.deepEqual(hero.getCombat(), { a: 5465, di: 4950, dc: 6370 }, 'combat');
            t.deepEqual(hero.getCost(), [27000, 23000, 33000, 9100], 'cost');
            t.equal(hero.getTime(), 128000, 'time');
            t.equal(hero.neededLvl, 39, 'lvl');
            t.equal(hero.neededExp, 78000, 'exp');
            t.end();
        });
        t.test('level 100', t => {
            ...
            t.equal(hero.neededExp, 495000, 'exp');
            t.end();
        });
        t.end();
    });
    t.test('Paladin', t => {
        ...
        const hero = new Hero(unit);
        t.test('level 0', t => {
            t.deepEqual(hero.getCombat(), { a: 70, di: 165, dc: 65 }, 'combat');
        ...
        t.test('level 40', t => {
            hero.setSkill('off', 100);
            hero.setSkill('def', 100);
            t.deepEqual(hero.getCombat(), { a: 6485, di: 10135, dc: 5025 }, 'combat');
