...
import { Place, Def, Off, CombatResult } from '../types';

import combat from './combat';
import units from './units';

function result(obj = {}): CombatResult {
    return extend<CombatResult>({
        offLosses: 0,
        defLosses: 0,
    }, obj);
}
...
tape('combat (e2e)', (t) => {
    ...
    /* t.test('catapults with remorale', t => {
        t.equal(tcombat.combat(
            place({ pop: 1000 }),
            [ def({ units: units[1], numbers: [0, 999999] }),
              off({ units: units[0], numbers: [0,670175,0, 0,0,0, 0,20], pop: 2000, targets: [5] }),
            ])[0].buildings, [0]);
        t.equal(tcombat.combat(
            place({ pop: 1000 }),
            [ def({ units: units[1], numbers: [0, 999999] }),
              off({ units: units[0], numbers: [0,670174,0, 0,0,0, 0,20], pop: 2000, targets: [5] }),
            ])[0].buildings, [1]);
        t.end();
    }); */
    t.end();
